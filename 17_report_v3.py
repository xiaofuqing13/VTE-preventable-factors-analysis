...
def sf_table(bdf, nn, np_, cat):
    ...
    for _, r in sig.iterrows():
        ...
        rows.append([r['变量'], ts, r[pc], r[nc], st, fv(r['P值'])])
...
# --- 2.2 多因素 ---
sec('2.2 多因素分析')
body('将单因素分析中P<0.05的变量纳入多因素Logistic回归。采用逐步回归法（Forward: LR），纳入标准P<0.05，剔除标准P>0.10。')
...
body(f'综合测试集评估指标（训练集采用5折交叉验证），模型性能排序为：{"  > ".join(_perf_strs)}。{_best["模型"]}综合判别性能最优，F1={_best.get("F1值",_best.get("F1",0)):.3f}，灵敏度={_best["灵敏度"]:.3f}。')
...
for ck in ['一般','疾病','治疗','生化']:
    ...
    cn = CN[ck]
    sub(f'{si} {cn}')
    rows, ns, ntot = sf_table(ab, 25, 63, ck)
    t = nt()
    cap(f'表{t} {cn}单因素分析结果')
    if rows:
...
for _, r in hb_sig.iterrows():
    ...
    hb_rows.append([r['变量'], ts, r[pc_hb], r[nc_hb], st, fv(r['P值'])])
t = nt(); cap(f'表{t} 3-31前HA-VTE规范预防单因素分析（P<0.05）')
mt(['变量','合计（n=153）\n例数（%）','规范预防组（n=46）\n例数（%）',
    '非规范预防组（n=107）\n例数（%）','统计量值\nχ²/t','P值'], hb_rows)
body(f'共{len(hbb)}个变量，{len(hb_sig)}个P<0.05。')
...
for _, r in ha_sig.iterrows():
    ...
    ha_rows.append([r['变量'], ts, r[pc_ha], r[nc_ha], st, fv(r['P值'])])
...
body(f'（五）机器学习（训练集5折交叉验证）：{_best_test["模型"]}最佳（测试集AUC={_best_test["AUC"]:.3f}），SHAP与统计分析结果高度一致。外部验证最佳AUC={_best_ext["AUC"]:.3f}（{_best_ext["模型"]}）。')
...
body('本分析已排除预防相关泄漏变量（76个），确保结果不受定义性变量污染。')
